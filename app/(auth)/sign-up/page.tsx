...
export default function SignUpPage() {
  ...
  async function handleSignUp(e: React.FormEvent) {
    ...
    const { error } = await supabase.auth.signUp({
      ...
      password,
      options: {
        // Profile row is created by a Supabase trigger (see migration).
        // Default: waitlist = true, is_admin = false.
    ...
    <main className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-sm space-y-6">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900">Join Fluent</h1>
          <p className="mt-1 text-sm text-gray-500">
            Get early access — we&apos;ll let you in as we open up spots.
          </p>
        </div>
